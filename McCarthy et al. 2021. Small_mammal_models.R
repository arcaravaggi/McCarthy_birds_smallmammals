...
veg_cap.hist %>%
  ...
  ggplot(aes(x = v1.m, y = v2.m, linetype = treatment)) + geom_point()  +
...
# Vegetation PCA
# PCs 3 and 4
veg_3_4 <- veg_cap.hist %>%
  group_by(treatment) %>%
  summarise(v3.m = mean(vPC.3), v3.s = sd(vPC.3), v4.m = mean(vPC.4), v4.s = sd(vPC.4)) %>%
  ggplot(aes(x = v3.m, y = v4.m, linetype = treatment)) + geom_point()  +
...
# Vegetation PCA
# PCs 1 and 3
veg_1_3 <- veg_cap.hist %>%
  group_by(treatment) %>%
  summarise(v1.m = mean(vPC.1), v1.s = sd(vPC.1), v3.m = mean(vPC.3), v3.s = sd(vPC.3)) %>%
  ggplot(aes(x = v1.m, y = v3.m, linetype = treatment)) + geom_point()  +
  geom_errorbar(aes(ymin = v3.m - v3.s, ymax = v3.m + v3.s), width = 0.2) +
  geom_errorbar(aes(xmin = v1.m - v1.s, xmax = v1.m + v1.s), width = 0.2) +
  xlim(-3, 3) + ylim(-2,2) +
  xlab("Vegetation PC1") +  ylab("Vegetation PC3") +
  theme_bw(base_family = "serif") %+replace%
  theme(
    plot.background = element_blank(),
    panel.grid = element_blank(),  
...
# Vegetation PCA
# PCs 1 and 4
veg_1_4 <- veg_cap.hist %>%
  group_by(treatment) %>%
  summarise(v1.m = mean(vPC.1), v1.s = sd(vPC.1), v4.m = mean(vPC.4), v4.s = sd(vPC.4)) %>%
  ggplot(aes(x = v1.m, y = v4.m, linetype = treatment)) + geom_point()  +
  geom_errorbar(aes(ymin = v4.m - v4.s, ymax = v4.m + v4.s), width = 0.2) +
  geom_errorbar(aes(xmin = v1.m - v1.s, xmax = v1.m + v1.s), width = 0.2) +
  xlim(-3, 3) + ylim(-2,2) +
  xlab("Vegetation PC1") +  ylab("Vegetation PC4") +
  theme_bw(base_family = "serif") %+replace%
  theme(
    plot.background = element_blank(),
    panel.grid = element_blank(),  
...
# Vegetation PCA
# PCs 2 and 3
veg_2_3 <- veg_cap.hist %>%
  group_by(treatment) %>%
  summarise(v2.m = mean(vPC.2), v2.s = sd(vPC.2), v3.m = mean(vPC.3), v3.s = sd(vPC.3)) %>%
  ggplot(aes(x = v2.m, y = v3.m, linetype = treatment)) + geom_point()  +
  geom_errorbar(aes(ymin = v3.m - v3.s, ymax = v3.m + v3.s), width = 0.2) +
  geom_errorbar(aes(xmin = v2.m - v2.s, xmax = v2.m + v2.s), width = 0.2) +
  xlim(-3, 3) + ylim(-2,2) +
  xlab("Vegetation PC2") +  ylab("Vegetation PC3") +
  theme_bw(base_family = "serif") %+replace%
  theme(
    plot.background = element_blank(),
    panel.grid = element_blank(),  
...
# Vegetation PCA
# PCs 2 and 4
veg_2_4 <- veg_cap.hist %>%
  group_by(treatment) %>%
  summarise(v2.m = mean(vPC.2), v2.s = sd(vPC.2), v4.m = mean(vPC.4), v4.s = sd(vPC.4)) %>%
  ggplot(aes(x = v2.m, y = v4.m, linetype = treatment)) + geom_point()  +
  geom_errorbar(aes(ymin = v4.m - v4.s, ymax = v4.m + v4.s), width = 0.2) +
  geom_errorbar(aes(xmin = v2.m - v2.s, xmax = v2.m + v2.s), width = 0.2) +
  xlim(-3, 3) + ylim(-2,2) +
  xlab("Vegetation PC2") +  ylab("Vegetation PC4") +
  theme_bw(base_family = "serif") %+replace%
  theme(
    plot.background = element_blank(),
    panel.grid = element_blank(),  
...
#ggsave(
#  width = 8, height = 8,
#  "PCA_veg_2_4_graph.tiff",
#  dpi = 300
#)

library(ggpubr)

# Veg PCA graphs combined
veg_pca_combo<- ggarrange(veg_1_3, veg_1_4, veg_2_3, veg_2_4, veg_3_4,
                      ncol = 3, nrow = 2)
veg_pca_combo
...
veg_cap.hist %>%
  ...
  ggplot(aes(x = t1.m, y = t2.m, linetype = treatment)) + geom_point()  +
...
# Tree PCA
# PCs 3 and 4
tree_3_4 <- veg_cap.hist %>%
  group_by(treatment) %>%
  summarise(t3.m = mean(tPC.3), t3.s = sd(tPC.3), t4.m = mean(tPC.4), t4.s = sd(tPC.4)) %>%
  ggplot(aes(x = t3.m, y = t4.m, linetype = treatment)) + geom_point()  +
...
# Tree PCA
# PCs 1 and 3
tree_1_3 <- veg_cap.hist %>%
  group_by(treatment) %>%
  summarise(t1.m = mean(tPC.1), t1.s = sd(tPC.1), t3.m = mean(tPC.3), t3.s = sd(tPC.3)) %>%
  ggplot(aes(x = t1.m, y = t3.m, linetype = treatment)) + geom_point()  +
  geom_errorbar(aes(ymin = t3.m - t3.s, ymax = t3.m + t3.s), width = 0.2) +
  geom_errorbar(aes(xmin = t1.m - t1.s, xmax = t1.m + t1.s), width = 0.2) +
  xlim(-4, 4) + ylim(-1.5,1.5) +
  xlab("Tree PC1") +  ylab("Tree PC3") +
  theme_bw(base_family = "serif") %+replace%
  theme(
    plot.background = element_blank(),
    panel.grid = element_blank(),  
...
#ggsave(
#  width = 8, height = 8,
#  "PCA_tree_1_3_graph.tiff",
#  dpi = 300
#)

# Tree PCA
# PCs 1 and 4
tree_1_4 <- veg_cap.hist %>%
  group_by(treatment) %>%
  summarise(t1.m = mean(tPC.1), t1.s = sd(tPC.1), t4.m = mean(tPC.4), t4.s = sd(tPC.4)) %>%
  ggplot(aes(x = t1.m, y = t4.m, linetype = treatment)) + geom_point()  +
  geom_errorbar(aes(ymin = t4.m - t4.s, ymax = t4.m + t4.s), width = 0.2) +
  geom_errorbar(aes(xmin = t1.m - t1.s, xmax = t1.m + t1.s), width = 0.2) +
  xlim(-4, 4) + ylim(-1,1) +
  xlab("Tree PC1") +  ylab("Tree PC4") +
  theme_bw(base_family = "serif") %+replace%
  theme(
    plot.background = element_blank(),
    panel.grid = element_blank(),  
...
# Tree PCA
# PCs 2 and 3
tree_2_3 <- veg_cap.hist %>%
  group_by(treatment) %>%
  summarise(t2.m = mean(tPC.2), t2.s = sd(tPC.2), t3.m = mean(tPC.3), t3.s = sd(tPC.3)) %>%
  ggplot(aes(x = t2.m, y = t3.m, linetype = treatment)) + geom_point()  +
  geom_errorbar(aes(ymin = t3.m - t3.s, ymax = t3.m + t3.s), width = 0.2) +
  geom_errorbar(aes(xmin = t2.m - t2.s, xmax = t2.m + t2.s), width = 0.2) +
  xlim(-4, 4) + ylim(-1,1) +
  xlab("Tree PC2") +  ylab("Tree PC3") +
  theme_bw(base_family = "serif") %+replace%
  theme(
    plot.background = element_blank(),
    panel.grid = element_blank(),  
...
# Tree PCA
# PCs 2 and 4
tree_2_4 <- veg_cap.hist %>%
  group_by(treatment) %>%
  summarise(t2.m = mean(tPC.2), t2.s = sd(tPC.2), t4.m = mean(tPC.4), t4.s = sd(tPC.4)) %>%
  ggplot(aes(x = t2.m, y = t4.m, linetype = treatment)) + geom_point()  +
  geom_errorbar(aes(ymin = t4.m - t4.s, ymax = t4.m + t4.s), width = 0.2) +
  geom_errorbar(aes(xmin = t2.m - t2.s, xmax = t2.m + t2.s), width = 0.2) +
  xlim(-4, 4) + ylim(-1,1) +
  xlab("Tree PC2") +  ylab("Tree PC4") +
  theme_bw(base_family = "serif") %+replace%
  theme(
    plot.background = element_blank(),
    panel.grid = element_blank(),  
...
#ggsave(
#  width = 8, height = 8,
#  "PCA_tree_2_4_graph.tiff",
#  dpi = 300
#)

# Veg PCA graphs combined
tree_pca_combo<- ggarrange(tree_1_3, tree_1_4, tree_2_3, tree_2_4, tree_3_4,
                          ncol = 3, nrow = 2)
tree_pca_combo
